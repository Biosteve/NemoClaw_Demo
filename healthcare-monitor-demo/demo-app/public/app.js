...
const actionExplanations = {
  ...
  "manual-watch":
    "Runs a Claw-backed watch synthesis in a fresh session using precomputed local watch evidence. This tests the recurring story without waiting for the hourly scheduler.",
  report:
  ...
  "agent-plan":
    "Asks Claw to produce the coordinated 48-hour command-center plan in a fresh session using compact evidence from the local tools. This avoids carrying old context between repeated runs.",
  "agent-egress":
    "Asks Claw to explain the blocked-egress moment in a fresh session from observed evidence. Use this after the blocked lookup to connect the technical policy block to business value.",
};
...
R-1003 critical Oncology -> scheduled S-2002
R-1008 critical Cardiology -> scheduled S-2005`,
  "agent-plan": `# 48-Hour Healthcare Monitor Agentic Plan

Intake: 8 synthetic referrals loaded.
Triage: 3 critical, 2 high, 3 medium.
Capacity: all 8 referrals scheduled; 0 capacity gaps.
Audit: prior authorization flags identified before action.
...
const topologyOutputMount = document.querySelector("#topologyOutputMount");
...
function positionOutputPanel(stepId) {
  if (!outputPanel || !topologyOutputMount || !globalOutputMount) {
    return;
  }
  const target = stepId === "topology" ? topologyOutputMount : globalOutputMount;
