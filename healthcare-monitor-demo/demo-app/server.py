...
def command_for(action: str) -> tuple[list[str], Path, int, bool] | None:
    ...
    no_delivery = "Do not call message, delivery, or channel-send tools. Return the answer directly in this CLI session only."
    ...
    if action == "manual-watch":
        message = one_line_message(
            "Write a concise scheduled healthcare-monitor watch update from this already computed evidence. "
            "The local evidence below is complete; answer directly without additional evidence collection. "
            f"Evidence: {run_command(['python3', 'workspace/scripts/care_backlog_analyzer.py', 'watch-summary'], RUNTIME_ROOT, 90)['stdout']} "
            f"{no_delivery}"
        )
        return (
            sandbox_exec
            + [
                ...
                message,
                "--session-id",
                fresh_session("healthcare-monitor-watch"),
            ],
            RUNTIME_ROOT,
            240,
            True,
    ...
    if action == "agent-plan":
        message = one_line_message(
            ...
            "Checklist: 1 call agents_list. 2 call sessions_spawn once with agentId intake and ask it to run the intake workflow. "
            ...
            "After item 6, answer directly in this CLI session with sections: Situation, Priority actions, Capacity plan, Audit/payer implications, Governance note. Synthetic data only; no medical advice. "
            f"{no_delivery}"
        )
        return (
            sandbox_exec
    ...
    if action == "agent-egress":
        message = one_line_message(
            "Explain the OpenShell blocked egress behavior for the healthcare monitor example. "
            "Use this observed evidence and answer directly without additional evidence collection. "
            f"Evidence: {compact_egress_evidence()} "
            "Explain why this matters for governed agent behavior. "
            f"{no_delivery}"
        )
        return (
            sandbox_exec
            + [
                ...
                message,
                "--session-id",
                fresh_session("healthcare-monitor-egress"),
            ],
            RUNTIME_ROOT,
            240,
            True,
            ...
            self.send_json(
                {
                    ...
                    "provider": provider_label(env.get("NEMOCLAW_PROVIDER", "custom")),
                    "model": env.get("NEMOCLAW_MODEL", "nvidia/nvidia/nemotron-3-super-v3"),
